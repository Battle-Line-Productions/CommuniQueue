  ...
  <div class="p-4 sm:p-6 lg:p-8 space-y-6">
    ...
    <div class="bg-light-surface dark:bg-dark-surface p-6 rounded-lg shadow-sm">
      ...
      <div v-else class="space-y-4">
        <UserManagementCard v-for="user in users" :key="user.id" :user="user" :can-manage="canManageUsers" :is-current-user="isCurrentUser(user.id)" @remove="handleUserRemove" />
      </div>
    </div>

    <!-- Add User Modal -->
    <UserManagementAddUserModal v-if="showAddUserModal" @close="showAddUserModal = false" @added="handleUserAdded" />

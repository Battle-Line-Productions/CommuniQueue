<template>
  <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
    <div class="bg-light-surface dark:bg-dark-surface p-6 rounded-lg w-full max-w-md">
      <h2 class="text-lg font-semibold mb-4 text-light-textbase dark:text-dark-textbase"> Add a New User </h2>

      <form class="space-y-4" @submit.prevent="submit">
        <!-- First Name -->
        <FormField id="addUserFirstName" v-model="firstName" label="First Name" required placeholder="John" :error="errors.firstName" @input="errors.firstName = ''" />

        <!-- Last Name -->
        <FormField id="addUserLastName" v-model="lastName" label="Last Name" required placeholder="Doe" :error="errors.lastName" @input="errors.lastName = ''" />

        <!-- Email -->
        <FormField id="addUserEmail" v-model="email" label="Email" required placeholder="user@example.com" :error="errors.email" @input="errors.email = ''" />

        <!-- Global Role (Owner not selectable) -->
        <FormField id="addUserGlobalRole" v-model="globalRole" label="Global Role" type="select" :required="true">
        ...
        <div class="flex items-center gap-2">
          <input id="addUserIsActive" v-model="isActive" type="checkbox" class="h-4 w-4 rounded border-light-secondary/20 dark:border-dark-secondary/20" >
          <label for="addUserIsActive" class="text-sm text-light-textbase dark:text-dark-textbase cursor-pointer"> Active User </label>
        </div>

        <!-- Buttons -->
        <div class="flex justify-end space-x-2 mt-6">
          <button type="button" class="px-4 py-2 text-light-secondary dark:text-dark-secondary hover:underline" @click="$emit('close')"> Cancel </button>
          <button :disabled="isPending" class="bg-light-primary dark:bg-dark-primary text-white px-4 py-2 rounded-md hover:opacity-90 transition-opacity disabled:opacity-50">

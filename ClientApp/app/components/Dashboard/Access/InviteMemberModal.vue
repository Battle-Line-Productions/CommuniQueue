<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" @click.self="$emit('close')">
    <div class="w-full max-w-md mx-4 bg-light-surface dark:bg-dark-surface rounded-lg shadow-xl transform transition-all duration-300 ease-in-out" role="dialog" aria-modal="true">
      <!-- Header -->
      <div class="flex justify-between items-center p-6 border-b border-light-secondary/10 dark:border-dark-secondary/10">
        <h2 class="text-xl font-semibold text-light-textbase dark:text-dark-textbase"> Invite Team Member </h2>
        <button class="text-light-secondary dark:text-dark-secondary hover:text-light-textbase dark:hover:text-dark-textbase" @click="$emit('close')">
      ...
      <div class="p-6 space-y-6">
        ...
        <div>
          ...
          <div class="relative">
            <FormField v-model="searchTerm" label="Email or Name" type="text" placeholder="Search by email or name" :disabled="isPending" />

            <!-- Loading State -->
            <div v-if="isSearching" class="absolute z-10 w-full mt-1 flex items-center justify-center py-2 text-light-secondary dark:text-dark-secondary">
              <LoadingSpinner class="h-5 w-5" />
              <span class="ml-2">Searching...</span>
            </div>

            <!-- Error State -->
            <div v-else-if="searchError" class="absolute z-10 w-full mt-1 bg-light-error/10 dark:bg-dark-error/10 text-light-error dark:text-dark-error p-2 rounded-md">
              <p class="text-sm">
                <Icon name="mdi:alert-circle" class="inline-block mr-2 w-5 h-5" />
            ...
            >
              ...
              >
                ...
                <Icon v-if="selectedUser?.id === user.id" name="mdi:check-circle" class="w-5 h-5 text-light-success dark:text-dark-success" />
              </div>
            </div>

            <!-- No Results State -->
            <div v-else-if="searchTerm.length >= 2" class="absolute z-10 w-full mt-1 px-4 py-2 text-light-secondary dark:text-dark-secondary"> No users found </div>
          </div>
        </div>

        <!-- Selected User Display -->
        <div v-if="selectedUser" class="bg-light-background dark:bg-dark-background p-4 rounded-lg flex items-center justify-between">
          ...
          <button type="button" class="text-light-error dark:text-dark-error hover:opacity-80" @click="clearSelectedUser">
        ...
        <div>
          ...
          <select
            ...
            :disabled="isPending"
          >
            <option v-for="level in availablePermissionLevels" :key="level.value" :value="level.value">
              {{ level.label }}
            </option>
          </select>
          ...
          <div class="mt-4 p-4 bg-light-background dark:bg-dark-background rounded-lg">
            <h4 class="font-medium text-light-textbase dark:text-dark-textbase mb-2"> {{ currentPermissionLevel.label }} Access </h4>
            <p class="text-sm text-light-secondary dark:text-dark-secondary">
              {{ currentPermissionLevel.description }}
            </p>
            <ul class="mt-2 space-y-1">
              <li v-for="(ability, index) in currentPermissionLevel.abilities" :key="index" class="text-sm flex items-center space-x-2 text-light-textbase dark:text-dark-textbase">
                <Icon name="mdi:check-circle" class="w-4 h-4 text-light-success dark:text-dark-success" />
                <span>{{ ability }}</span>
              </li>
            </ul>
          </div>
        ...
        <div class="flex justify-end space-x-4 pt-4">
          <button type="button" class="text-light-secondary dark:text-dark-secondary hover:underline px-4 py-2" :disabled="isPending" @click="$emit('close')"> Cancel </button>
          <button
            type="button"

  ...
  <div class="space-y-6">
    <div class="bg-light-surface dark:bg-dark-surface p-6 rounded-lg shadow-sm">
      <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-semibold text-light-textbase dark:text-dark-textbase">Team Members</h2>
        <button
          v-if="canManageUsers"
      ...
      <div v-else-if="projectUsers?.length === 0" class="text-center py-8">
        <Icon name="mdi:account-group-outline" class="w-16 h-16 mx-auto text-light-secondary dark:text-dark-secondary mb-2" />
        <p class="text-light-secondary dark:text-dark-secondary">No team members yet</p>
      </div>
    ...
    <DashboardAccessInviteMemberModal v-if="showInviteModal" :project-id="projectId" @close="showInviteModal = false" @invited="handleInvited" />
